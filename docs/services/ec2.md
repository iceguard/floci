...
Before releasing native first boot, Floci installs the local ACM authority and certificate-chain entries held by emulated ACM into the guest system trust store. Amazon-issued chains are trusted normally. Imported chains are trusted only when their certificate is scoped to Floci's configured local hostname namespace, allowing local certificate fixtures to model public roots present in real Ubuntu provider images without making arbitrary imported private authorities trusted by compute guests. Leaf certificate bodies and private keys are never copied into guests.
