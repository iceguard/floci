...
Before releasing native first boot, Floci installs the certificate-chain entries held by emulated ACM into the guest system trust store. This models the public trust roots already present in real Ubuntu provider images so application bootstrap can keep strict TLS verification against Floci-issued endpoints. Leaf certificate bodies and private keys are never copied into guests.
